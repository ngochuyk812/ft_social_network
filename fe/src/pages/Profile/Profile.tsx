...
import { Button, Image, Input, Modal, Result } from "antd";
function Profile() {
...
const PostTab = ({ id }: { id: string }) => {
    ...
    const { data, isFetching } = useGetPostByPageAndUserQuery({ index: page, idUser: Number(id) });
...
const MediaTab = ({ id }: { id: string }) => {
    const { data } = useGetMediaUserQuery({ idUser: Number(id) })
    const render = () => {
